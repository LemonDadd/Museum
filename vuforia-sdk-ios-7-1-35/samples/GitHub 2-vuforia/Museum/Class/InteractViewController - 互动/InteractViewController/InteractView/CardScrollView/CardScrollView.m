...
- (void)loadCard {
    ...
    for (NSInteger index = 0; index < (self.totalNumberOfCards < 4 ? self.totalNumberOfCards : 4); index++) {
        ...
        UITapGestureRecognizer *gr = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(seletedCard:)];
        [card addGestureRecognizer:gr];
...
- (void)seletedCard:(UITapGestureRecognizer *)gr {
    if ([_cardDelegate respondsToSelector:@selector(didSeletadCardViewWithIndx:)]) {
        [_cardDelegate didSeletadCardViewWithIndx:gr.view.tag];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat progress = fabs(diff)/(kGCViewWidth*1);
    ...
    if (fabs(diff) >= kGCScrollViewWidth*0.5) {
