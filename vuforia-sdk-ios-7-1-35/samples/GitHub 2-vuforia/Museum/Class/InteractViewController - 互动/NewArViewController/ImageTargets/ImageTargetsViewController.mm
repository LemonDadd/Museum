...
@interface ImageTargetsViewController ()<ARModelViewControllerDelegate>
...
- (void)loadView//加载视图
{
    ...
    AppDelegate *appDelegate = (AppDelegate*)[[UIApplication sharedApplication] delegate];//Vuforia的样本代理
    ...
    // initialize AR 初始化AR
    [vapp initAR:Vuforia::GL_20 orientation:self.interfaceOrientation deviceMode:Vuforia::Device::MODE_AR stereo:false];
...
- (void)viewDidLoad
{
    ...
    //跳转
    NSString *plistPath = [[NSBundle mainBundle] pathForResource:@"ArScan" ofType:@"plist"];
    _allResource = [[NSArray array]initWithContentsOfFile:plistPath];
    
    
    
    CGFloat width = 120.f*([UIScreen mainScreen].bounds.size.width/320.0);
    CGFloat hight = [DeviceInfo scalHeightByWidth:width BySize:[UIImage imageNamed:@"ImageTargets_logo.JPG"].size];
    _logoImageV = [UIImageView new];
    _logoImageV.image = [UIImage imageNamed:@"ImageTargets_logo.JPG"];
    ...
    UITapGestureRecognizer *gr = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(test)];
    [_logoImageV addGestureRecognizer:gr];
...
- (void)viewDidDisappear:(BOOL)animated {
    TT_INVALIDATE_TIMER(_rssisTimer);
...
- (bool) doLoadTrackersData {//做负载跟踪数据
    ...
    dataSetStonesAndChips = [self loadObjectTrackerDataSet:@"museumproject.xml"];//加载对象跟踪数据集
...
-(void)rssis2S{
    if ([self.GWLstring isEqualToString:@"y"]) {
        ...
        ArScanModel *model = nil;
        for (NSDictionary *dic in _allResource) {
            @autoreleasepool {
                if ([self.GWLNameID isEqualToString:dic[@"Name"]]) {
                    model = [ArScanModel mj_objectWithKeyValues:dic];
                    break;
                }
            }
        }
...
#pragma mark 关闭界面消失的时候关闭数据222205
...
- (void) scanlineCreate {
    ...
    [scanLineView setImage:[UIImage imageNamed:@"scanline"]];
