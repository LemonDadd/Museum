...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _registerBtn.titleLabel.font = [UIFont fontWithName:MFONT size:20];
        ...
        [_registerBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(_passWordView.mas_bottom).offset(kHeight(4*both));
            make.height.mas_equalTo(kHeight(45));
        }];
        
        UILabel *login = [UILabel new];
        login.font = [UIFont fontWithName:MFONT size:14];
        login.text = @"登录";
        login.textColor = kColorWithHex(0xa8a8a8);
        [self addSubview:login];
        [login mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(_registerBtn);
            make.top.equalTo(_registerBtn.mas_bottom);
        }];
        ...
        [wangji mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(_registerBtn);
            make.top.equalTo(_registerBtn.mas_bottom);
        }];
...
- (void)sendMsg {
    ...
    [SMSSDK getVerificationCodeByMethod:SMSGetCodeMethodSMS phoneNumber:_phoneView.textField.text zone:@"86" template:@"12163482" result:^(NSError *error) {
