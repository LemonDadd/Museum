...
- (instancetype)init
{
    ...
    if (self) {
        ...
        CGFloat width = [UILabel labelConstrainedToSize:@"手机号" font:[UIFont fontWithName:MFONT size:18]].width;
        
        _titleView = [UILabel new];
        _titleView.font =  [UIFont fontWithName:MFONT size:18];
        ...
        _textField.font = [UIFont fontWithName:MFONT size:16];
...
-(void)setShowMsgBtn:(BOOL)showMsgBtn {
    ...
    if (showMsgBtn) {
        ...
        _msgLabel.font =  [UIFont fontWithName:MFONT size:16];
        ...
        [_msgBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(_msgLabel);
        }];
        
        [_textField mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.bottom.equalTo(_titleView);
            make.left.equalTo(_titleView.mas_right);
            make.right.equalTo(_msgLabel.mas_left).offset(-10);
        }];
        [self layoutIfNeeded];
    }
}
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                _msgLabel.text  =[NSString stringWithFormat:@"%zds",(long)timeout];
