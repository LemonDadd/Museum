...
- (void)uploadMutableImageByUrlString:(NSString *)urlString
    ...
    if (images.count > 0) {
        ...
        if ([UserInfoClass getUserInfoClass].token && [UserInfoClass getUserInfoClass].token.length ) {
            [manager.requestSerializer setValue:[UserInfoClass getUserInfoClass].token forHTTPHeaderField:@"Authorization"];
        }
